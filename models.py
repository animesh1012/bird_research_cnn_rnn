...
class LegendreDelay:
    def __init__(self, theta=1.0, order=6):
        self.theta = theta
        self.order = order
...
class LMUCell(nn.Module):
    def __init__(self,
        ...
        factory_obj = factory(theta=theta, order=self.order)
        A, B, C, D = factory_obj.realization()
        system = (A, B, C, D)
        self._ss = cont2discrete(system, dt=1.0, method=method)
...
class cnn_rnn(nn.Module):
    ...
    ##Extract feature
    def extract_features(self, x, hidden_state):
        """
        Returns 512-dim learned representation before final classifier.
        """
        batch_size, timesteps, C, H, W = x.size()
